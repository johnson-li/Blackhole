...
class TraceRoute {
    ...
    fun traceRoute(hostname: String, async: Boolean = false): TraceRouteResult? {
        val args = arrayOf("traceroute", hostname, "-n", "-w1", "-m10")
        if (async) {
            Thread({
                traceRoute(args)
...
open class TraceRouteCallback {
     ...
     */
    open fun onSuccess(traceRouteResult: TraceRouteResult) {}
     ...
     */
    open fun onFailed(code: Int, reason: String) {}
