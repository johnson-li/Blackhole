package com.xuebingli.blackhole.utils

class Preferences {
    companion object {
        const val PREFERENCE_NAME = "preference"
        const val MEASUREMENT_SETUP_KEY = "measurement setup"
        const val CLIENT_ID_KEY = "client id"
        const val TARGET_IP_KEY = "target ip"
        const val LOGGING_KEY = "logging"
        const val POUR_MODE_KEY = "pour mode"
        const val SINK_MODE_KEY = "sink mode"
        const val INTERFACE_KEY = "interface"
        const val SINK_BITRATE_KEY = "sink bitrate"
        const val POUR_BITRATE_KEY = "pour bitrate"
        const val BITRATE_KEY = "bitrate"
        const val DATARATE_KEY = "datarate"
        const val PACKET_SIZE_KEY = "packet size"
        const val DURATION_KEY = "duration"
        const val FREQUENCY_KEY = "frequency"
        const val CLOCK_DRIFT_KEY = "clock drift"
        const val PROBING_DELAY_KEY = "probing delay"
        const val CLOCK_CONFIDENCE_KEY = "clock confidence"
        const val SERVICE_LOCATION = "location service"
        const val SERVICE_CELL_INFO = "cell info"
        const val SUBSCRIPTION_INFO = "subscription info"
    }
}